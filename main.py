...
# introduction about this kernel
version = 5
basic_name = f'Unet_resnet_v{version}'
save_model_name = basic_name + '.model'
submission_file = basic_name + '.csv'

# create onject for datasets
class TGSSaltDatasets(obejct):
	...
	def __getitem__(self, index):
		...
		image = np.array(imread(image_path), dtype=uint8)
		mask = np.array(imread(mask_path), dtype=uint8)

		return image, mask

	def _getitem(self, img_id):
		#image folder
		image_folder = os.path.join(self.root_path, "images")
...
depths_df = pd.read_csv("depths", index_col="id")
...
def visualization():
	...
	for _, file_id in enumerate(ids):
		image, mask = dataset._getitem(file_id)
		plot2x2Array(image, mask)

	# display images on the screen using OpenCV

	import cv2
	from Ipython.display import display, Image
	def cvshow(image, format='.png', rate=255):
		decode_bytes = cv2.imencode(format, image*rate)[1].tobytes()
		display(Image(data=decode_bytes))
		return
	
	def arangingImgInTileShape():
		x_train_sorted_by_coverage=np.array(train_df.sort_values(by='coverage').images.tolist()).reshape(-1, img_size_target, img_size_target, 1)
		x_train_sorted_by_z=np.array(train_df.sort_values(by='z').images.tolist()).reshape(-1, img_size_target, img_size_target, 1)
		y_train_sorted_by_coverage=np.array(train_df.sort_values(by='coverage').masks.tolist()).reshape(-1, img_size_target, img_size_target, 1)
		y_train_sorted_by_z=np.array(train_df.sort_values(by='z').masks.tolist()).reshape(-1, img_size_target, img_size_target, 1)
...
train_df["coverage_class"] = train_df.coverage.map(cov_to_class)#decrete data in order to train_test_split
# explore data
...
# data augmentation by padding four sides of an images into mirror image
def create_padded(imgs,dst_hw,odd_mirror=True):
	...
	return ret
# randomly cut out images using Keras's callback function
...
# visualization train_loss, validation_loss
...
x_test = np.array([(np.array(load_img("../input/test/images/{}.png".format(idx),
			grayscale = True))) / 255 for idx in tqdm_notebook(test_df.index)]).reshape(-1, img_size_target, img_size_target, 1)
